...
uint8_t Serial_RxData_1;
uint8_t Serial_RxFlag_1;

void Serial_Init(void)
{
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, ENABLE);      //打开总线上的UART时钟与RX,TX引脚时钟
	...
	USART_Cmd(USART1, ENABLE);                                                         //使能UART1开始运行
}

void Serial_SendByte(uint8_t Byte)                                                     //通过UART1发送一个字节
{
	USART_SendData(USART1, Byte);
	while(USART_GetFlagStatus(USART1, USART_FLAG_TXE) == RESET);
}

void Serial_SendArray(uint8_t *Array, uint16_t Length)                                 //发送数组中每一个字节
{
...
void Serial_SendNum(uint32_t Number, uint8_t Length)                                   //发送数字以字符串形式
{
	uint8_t i;
	for (i = 0; i < Length; i++)
	{
...
uint8_t Serial_GetRxFlag(void)                                                         //发送完成的标志
{
...
uint8_t Serial_GetRxData(void)                                                         //获取接收的数据
{
	return Serial_RxData_1;
}

void USART1_IRQHandler()                     // 串口 1 接收中断
{
	if(USART_GetFlagStatus(USART1, USART_IT_RXNE) == SET)
	{
		Serial_RxData_1 = USART_ReceiveData(USART1);

		uint8_t idx = Serial_RxData_1 % 16;
		Receive_Data[idx] = Serial_RxData_1 / 16;
